...
const Web3Provider = ({children})=>{
  ...
  const handleWallet = async()=>{
    try{
        const {contractInstance,selectedAccount,chainId,signer,provider,network,storedAddress} = await getWeb3State();
        setWeb3State({contractInstance,selectedAccount,chainId,signer,provider,network,storedAddress})
    }catch(error){
        console.error(error)
    }
  }
